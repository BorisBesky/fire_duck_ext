...
static unique_ptr<FunctionData> FirestoreInsertBind(
...
static unique_ptr<GlobalTableFunctionState> FirestoreInsertInitGlobal(
    ClientContext &context,
...
// Local state (minimal)
static unique_ptr<LocalTableFunctionState> FirestoreInsertInitLocal(
...
static void FirestoreInsertFunction(
    ClientContext &context,
...
static unique_ptr<FunctionData> FirestoreUpdateBind(
    ClientContext &context,
    TableFunctionBindInput &input,
    vector<LogicalType> &return_types,
    vector<string> &names
    ...
    if ((input.inputs.size() - 2) % 2 != 0) {
        throw BinderException("firestore_update requires field name/value pairs. "
...
static unique_ptr<GlobalTableFunctionState> FirestoreUpdateInitGlobal(
    ClientContext &context,
    TableFunctionInitInput &input
) {
...
static void FirestoreUpdateFunction(
    ClientContext &context,
    TableFunctionInput &data,
    DataChunk &output
) {
    ...
    try {
        ...
        for (size_t i = 0; i < bind_data.field_names.size(); i++) {
            fields[bind_data.field_names[i]] = DuckDBValueToFirestore(
                bind_data.field_values[i],
                bind_data.field_values[i].type()
            );
        }
...
static unique_ptr<FunctionData> FirestoreDeleteBind(
    ClientContext &context,
    TableFunctionBindInput &input,
    vector<LogicalType> &return_types,
    vector<string> &names
...
static unique_ptr<GlobalTableFunctionState> FirestoreDeleteInitGlobal(
    ClientContext &context,
    TableFunctionInitInput &input
) {
...
static void FirestoreDeleteFunction(
    ClientContext &context,
    TableFunctionInput &data,
    DataChunk &output
) {
...
static unique_ptr<FunctionData> FirestoreUpdateBatchBind(
    ClientContext &context,
    TableFunctionBindInput &input,
    vector<LogicalType> &return_types,
    vector<string> &names
...
static unique_ptr<GlobalTableFunctionState> FirestoreUpdateBatchInitGlobal(
    ClientContext &context,
    TableFunctionInitInput &input
) {
...
static void FirestoreUpdateBatchFunction(
    ClientContext &context,
    TableFunctionInput &data,
    DataChunk &output
) {
    ...
    try {
        ...
        for (size_t i = 0; i < bind_data.document_ids.size(); i++) {
            ...
            if (use_individual_ops) {
                ...
                std::string doc_path = "projects/" + bind_data.credentials->project_id +
                                       "/databases/" + bind_data.credentials->database_id + "/documents/" + collection + "/" + doc_id;
                ...
                json write_op = {
                    ...
                    {"updateMask", {
                        {"fieldPaths", bind_data.field_names}
                    }}
                };
...
static unique_ptr<FunctionData> FirestoreDeleteBatchBind(
    ClientContext &context,
    TableFunctionBindInput &input,
    vector<LogicalType> &return_types,
    vector<string> &names
...
static unique_ptr<GlobalTableFunctionState> FirestoreDeleteBatchInitGlobal(
    ClientContext &context,
    TableFunctionInitInput &input
) {
...
static void FirestoreDeleteBatchFunction(
    ClientContext &context,
    TableFunctionInput &data,
    DataChunk &output
) {
...
void RegisterFirestoreWriteFunctions(ExtensionLoader &loader) {
    ...
    TableFunction insert_func("firestore_insert",
                              {LogicalType::VARCHAR},  // collection
                              FirestoreInsertFunction,
                              FirestoreInsertBind,
                              FirestoreInsertInitGlobal,
    ...
    TableFunction update_func("firestore_update",
                              ...
                              FirestoreUpdateFunction,
                              FirestoreUpdateBind,
                              FirestoreUpdateInitGlobal);
    ...
    TableFunction delete_func("firestore_delete",
                              {LogicalType::VARCHAR, LogicalType::VARCHAR},  // collection, document_id
                              FirestoreDeleteFunction,
                              FirestoreDeleteBind,
                              FirestoreDeleteInitGlobal);
    ...
    // Usage: SELECT * FROM firestore_update_batch('collection', ['id1','id2'], 'field1', value1, ...)
    TableFunction update_batch_func("firestore_update_batch",
                                    {LogicalType::VARCHAR, LogicalType::LIST(LogicalType::VARCHAR)},
                                    FirestoreUpdateBatchFunction,
                                    FirestoreUpdateBatchBind,
                                    FirestoreUpdateBatchInitGlobal);
    ...
    // Usage: SELECT * FROM firestore_delete_batch('collection', ['id1','id2'])
    TableFunction delete_batch_func("firestore_delete_batch",
                                    {LogicalType::VARCHAR, LogicalType::LIST(LogicalType::VARCHAR)},
                                    FirestoreDeleteBatchFunction,
                                    FirestoreDeleteBatchBind,
                                    FirestoreDeleteBatchInitGlobal);
