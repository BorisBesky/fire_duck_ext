...
public:
    ...
    // Core logging method
    void Log(FirestoreLogLevel level, const std::string& message,
             const char* file = nullptr, int line = 0,
             const char* function = nullptr);

    // Convenience methods
    void Debug(const std::string& msg, const char* file = nullptr,
               int line = 0, const char* func = nullptr) {
        Log(FirestoreLogLevel::DEBUG, msg, file, line, func);
    }

    void Info(const std::string& msg, const char* file = nullptr,
              int line = 0, const char* func = nullptr) {
        Log(FirestoreLogLevel::INFO, msg, file, line, func);
    }

    void Warn(const std::string& msg, const char* file = nullptr,
              int line = 0, const char* func = nullptr) {
        Log(FirestoreLogLevel::WARN, msg, file, line, func);
    }

    void Error(const std::string& msg, const char* file = nullptr,
               int line = 0, const char* func = nullptr) {
...
#define FS_LOG_DEBUG_IF(cond, msg) do { if (cond) { FS_LOG_DEBUG(msg); } } while (0)
#define FS_LOG_INFO_IF(cond, msg) do { if (cond) { FS_LOG_INFO(msg); } } while (0)
#define FS_LOG_WARN_IF(cond, msg) do { if (cond) { FS_LOG_WARN(msg); } } while (0)
#define FS_LOG_ERROR_IF(cond, msg) do { if (cond) { FS_LOG_ERROR(msg); } } while (0)
