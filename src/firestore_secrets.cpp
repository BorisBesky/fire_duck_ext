...
static unique_ptr<BaseSecret> CreateFirestoreSecretFromConfig(
    ...
    throw InvalidInputException(
        "firestore secret requires either 'service_account_json' or 'api_key'"
    );
}

// Deserialize secret from storage
static unique_ptr<BaseSecret> FirestoreSecretDeserialize(
...
std::shared_ptr<FirestoreCredentials> GetFirestoreCredentialsFromSecret(
    ClientContext &context,
    ...
    if (auth_type == "service_account") {
        ...
        creds = FirestoreAuthManager::CreateApiKeyCredentials(
            project_id, api_key_it->second.ToString());
    } else {
...
std::shared_ptr<FirestoreCredentials> ResolveFirestoreCredentials(
    ClientContext &context,
    ...
    // Priority 2: Explicit API key (requires project_id)
        ...
        creds = FirestoreAuthManager::CreateApiKeyCredentials(
            project_id.value(), api_key.value());
    }
